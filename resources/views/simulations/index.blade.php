...
<div class="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 dark:from-gray-950 dark:to-gray-900">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="text-center mb-8">
            <h1 class="text-3xl font-bold text-white">Simulasi Keamanan Siber</h1>
            <p class="mt-2 text-gray-400">Latih keterampilan keamanan siber Anda melalui simulasi interaktif</p>
        </div>
            ...
            @foreach($simulations as $simulation)
                <div class="bg-gray-800/50 backdrop-blur-sm rounded-lg p-6 border border-gray-700/50 hover:border-blue-500/30 transition-all duration-300">
                    <div class="flex items-start justify-between mb-4">
                        <div>
                            <h3 class="text-xl font-semibold text-white">{{ $simulation->title }}</h3>
                            <p class="text-gray-400 mt-1">{{ $simulation->description }}</p>
                        </div>
                    ...
                    <div class="flex flex-col space-y-4">
                        <div class="flex items-center text-sm text-gray-400">
                            ...
                            <span>{{ count($simulation->steps) }} Langkah</span>
                        </div>
                        
                        @if(isset($userResults[$simulation->id]))
                            <div class="bg-gray-700/30 rounded-lg p-4 border border-gray-600/30">
                                <div class="flex items-center justify-between mb-2">
                                    <span class="text-sm font-medium text-gray-300">Nilai Terakhir</span>
                                    <span class="text-lg font-bold {{ $userResults[$simulation->id]->score >= 70 ? 'text-green-400' : 'text-red-400' }}">
                                        {{ $userResults[$simulation->id]->score }}%
                                    </span>
                                </div>
                                <div class="flex items-center text-sm text-gray-400">
                                    <i class="fas fa-clock mr-2 text-blue-400"></i>
                                    <span>{{ $userResults[$simulation->id]->completed_at->diffForHumans() }}</span>
                                </div>
                            </div>
                            
                            <a href="{{ route('simulations.start', $simulation) }}" 
